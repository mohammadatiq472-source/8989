...
2. **区划层级（含空间边界）**：州(13+1) → 郡(94, 含BBox) → 县(316, 含BBox) → 城市(2362+) → 地块(邻接图)。94郡和316县均有中心坐标+矩形边界(min/max)，ID编码规律清晰(junxian_id÷100=region_id)，可直接用于 Godot 地图生成
...
4. **3D 模型/着色器**：设备上 25,897 个 3D 文件未拉取，且项目当前前端主线已切到 Godot
...
| **3D 景观贴图** (landscape_png) | 48 | 用于 NeoX 3D 地形，可参考 Godot 地形方案 | 提取地形颜色/纹理参考值 |
...
| **PNG 3D 纹理** (png_direct 大部分) | ~250 | NeoX 3D 引擎专用纹理，需评估 Godot 可用性 |
