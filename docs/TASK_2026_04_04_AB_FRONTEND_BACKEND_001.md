...
2. Godot 客户端支持可配置后端地址，避免硬编码 localhost。

## 模块映射

- A（后端）：`M01 Runtime Gateway` + `M03 Planning and Commander`
- B（前端/Godot）：`M15 Frontend Command Workspace` + `M16 Frontend Map Rendering`
...
- 分支：`codex/ab-task1-godot-backend-endpoint`
- 路径白名单：
